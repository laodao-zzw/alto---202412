...
const YOUR_CONTENT_API_KEY='';
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        /**
         * Function to render the selected post's content in Ghost template format.
          ...
          if (post) {
              ...
              const primaryTag = post.primary_tag
                  ? `<span class="post-meta-item post-meta-tags">
                      <a class="post-tag post-tag-${post.primary_tag.slug}" href="/tag/${post.primary_tag.slug}" title="${post.primary_tag.name}">
                          ${post.primary_tag.name}
                      </a>
                    </span>`
                  : '';

              const authorBox = post.authors && post.authors.length
              ...
              const relatedPosts = post.related_posts && post.related_posts.length
                  ? `<section class="related-posts">
                      <h3 class="related-title">Related Posts</h3>
                      <ul>
                          ${post.related_posts.map(rp => `<li><a href="${rp.url}">${rp.title}</a></li>`).join('')}
                      </ul>
                    </section>`
                  : '';
              ...
              contentElement.innerHTML = `
                  <div class="content-area">
                      <main class="site-main">
                          <article class="post ${post.feature_image ? '' : 'no-image'} single-post">
                              <header class="article-header gh-canvas">
                                  <div class="post-meta">
                                      <span class="post-meta-item post-meta-date">
                                          <time datetime="${new Date(post.published_at).toISOString().split('T')[0]}">
                                              ${new Date(post.published_at).toLocaleDateString()}
                                          </time>
                                      </span>
                                      ${post.reading_time ? `<span class="post-meta-item post-meta-length">${post.reading_time} min read</span>` : ''}
                                      ${primaryTag}
                                  </div>
